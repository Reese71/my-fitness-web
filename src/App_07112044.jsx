...
const App = () => {
  ...
  const [pendingCancellationSlots, setPendingCancellationSlots] = useState([]);
  ...
  const [mobileMovingWizardItem, setMobileMovingWizardItem] = useState(null);
  const [isEditingPublishedSchedule, setIsEditingPublishedSchedule] = useState(false);
  const [isViewingPublishedSchedule, setIsViewingPublishedSchedule] = useState(false);
  const [suppressPublishedAutoView, setSuppressPublishedAutoView] = useState(false);
  const [deletedPublishedBookingIds, setDeletedPublishedBookingIds] = useState([]);
  const [wizardAvailable, setWizardAvailable] = useState([]);
  const [wizardAvailableRangeKey, setWizardAvailableRangeKey] = useState('');
  const [wizardSelectedStudentId, setWizardSelectedStudentId] = useState('');
  ...
  useEffect(() => {
    ...
    const currentWeek = getWeekDaysFromCurrentMonday(new Date());
    const rangeStart = new Date(currentWeek[0]);
    ...
    const unsubscribeBookings = onSnapshot(
      ...
      (error) => {
        console.error('讀取 studentBookings 失敗：', error);
      }
    );
    ...
    const unsubscribeUnavailable = onSnapshot(
      ...
      (error) => {
        console.error('讀取 coachUnavailable 失敗：', error);
      }
    );
      ...
      (snapshot) => {
        const data = snapshot.docs.map((d) => {
          ...
          return {
            ...
            price: docData.price || '',
            weeklyLessons:
              docData.weeklyLessons ??
              docData.lessonsPerWeek ??
              1,
            ...docData
          };
        });
  ...
  useEffect(() => {
    const updateDeviceMode = () => {
      ...
      setIsMobileDevice(Boolean(hasCoarsePointer || isNarrowScreen));
    };
  ...
  const currentDateStr = formatLocalDate(currentNow);

  useEffect(() => {
    if (!selectedStudentId || mode !== 'student') {
      setStudentAvailable([]);
      return;
    }

    const baseDate = new Date(`${currentDateStr}T00:00:00`);
    const currentWeek = getWeekDaysFromCurrentMonday(baseDate);
    ...
    const selectedStudentQuery = query(
      collection(db, 'studentAvailable'),
      where('studentId', '==', selectedStudentId),
      where('time', '>=', Timestamp.fromDate(rangeStart)),
      where('time', '<', Timestamp.fromDate(rangeEnd))
    );

    const unsubscribeStudentAvailable = onSnapshot(
      ...
      (error) => {
        console.error('讀取選中學生的選課資料失敗：', error);
      }
    );

    return () => unsubscribeStudentAvailable();
  }, [selectedStudentId, currentDateStr, mode]);
  ...
  const twoWeekDays = useMemo(
    () => getTwoWeekDaysFromCurrentMonday(new Date(`${currentDateStr}T00:00:00`)),
    [currentDateStr]
  );

  const currentWeekDays = useMemo(
    () => getWeekDaysFromCurrentMonday(new Date(`${currentDateStr}T00:00:00`)),
    [currentDateStr]
  );

  const nextWeekDays = useMemo(
    () => getNextWeekDaysFromCurrentMonday(new Date(`${currentDateStr}T00:00:00`)),
    [currentDateStr]
  );

  const nextTwoWeekDateStrs = useMemo(
    () => getNextTwoWeekDays(new Date(`${currentDateStr}T00:00:00`)).map(formatLocalDate),
    [currentDateStr]
  );
  ...
  const publishedNextWeekBookings = useMemo(() => {
    return studentBookings
      .filter((booking) => nextWeekDateStrs.includes(booking.dateStr))
      .sort((a, b) => {
  ...
  }, [studentBookings, nextWeekDateStrs]);

  // 進入排課小精靈時，若下週已有正式排課，預設直接顯示已儲存內容。
  // 但使用者按下「取消編輯並清空畫面」後，暫時不要自動把正式排課載回來，
  // 讓下一次重新產生可以把整週視為空白重新排。
  useEffect(() => {
    if (
      adminTab !== 'wizard' ||
      isEditingPublishedSchedule ||
      suppressPublishedAutoView
    ) {
      return;
    }

    if (publishedNextWeekBookings.length > 0) {
      const publishedItems = publishedNextWeekBookings.map((booking) => ({
        ...booking,
      ...
      setWizardPlan(publishedItems);
      setWizardUnscheduled([]);
      setWizardSelectedStudentId('');
      setIsViewingPublishedSchedule(true);
      return;
    }

    setIsViewingPublishedSchedule(false);
  }, [
    adminTab,
    publishedNextWeekBookings,
    isEditingPublishedSchedule,
    suppressPublishedAutoView
  ]);

  // 離開排課小精靈後解除暫時抑制；下次重新進入時仍會正常顯示已儲存課表。
  useEffect(() => {
    if (adminTab !== 'wizard') {
      setSuppressPublishedAutoView(false);
    }
  }, [adminTab]);
  ...
  const togglePendingAvailabilitySlot = (slot) => {
    ...
    const existingItem = studentAvailable.find((item) => isSameSlot(item, slot));

    // 已送出的綠色時段：切換成「待取消」
    if (existingItem) {
      const isMarkedForCancellation = pendingCancellationSlots.some(
        (item) => item.id === existingItem.id
      );

      setPendingCancellationSlots((prev) => {
        if (isMarkedForCancellation) {
          return prev.filter((item) => item.id !== existingItem.id);
        }

        return [...prev, existingItem].sort((a, b) => {
          if (a.dateStr !== b.dateStr) return a.dateStr.localeCompare(b.dateStr);
      ...
      setPendingAvailabilitySlots((prev) =>
        prev.filter((item) => !isSameSlot(item, slot))
      );
      return;
    }

    // 尚未送出的時段：切換成「待新增」
    const isSelected = pendingAvailabilitySlots.some((item) =>
  ...
  const handleConfirmAvailability = async () => {
    ...
    if (
      pendingAvailabilitySlots.length === 0 &&
      pendingCancellationSlots.length === 0
    ) {
      alert('請先選擇要新增或取消的可上課時間');
      return;
    }

    const newItems = pendingAvailabilitySlots.filter((slot) => {
      return !studentAvailable.some((item) => isSameSlot(item, slot));
    });
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        pendingCancellationSlots.forEach((item) => {
          transaction.delete(doc(db, 'studentAvailable', item.id));
        });
      });

      const messages = [];
      if (newItems.length > 0) {
        messages.push(`新增 ${newItems.length} 個可上課時間`);
      }
      if (pendingCancellationSlots.length > 0) {
        messages.push(`取消 ${pendingCancellationSlots.length} 個可上課時間`);
      }

      alert(`已完成：${messages.join('、')}`);
      setPendingAvailabilitySlots([]);
      setPendingCancellationSlots([]);
    } catch (error) {
      console.error('更新學生可上課時間失敗：', error);
      alert(error.message);
    }
  };
  ...
  const wizardBaseBookings = useMemo(() => {
    // 產生或重新產生下週課表時，資料庫中既有的下週正式排課
    // 不應再被當成已占用時段，否則舊課表會先把學生可用時間卡住，
    // 新建議只能從剩餘時段中挑選。
    //
    // 只有單純「查看已儲存正式排課」時，才保留完整 studentBookings。
    if (isViewingPublishedSchedule && !isEditingPublishedSchedule) {
      return studentBookings;
    }

    return studentBookings.filter(
      (booking) => !nextWeekDateStrs.includes(booking.dateStr)
    );
  }, [
    studentBookings,
    nextWeekDateStrs,
    isViewingPublishedSchedule,
    isEditingPublishedSchedule
  ]);
  ...
  const scoreCandidate = (candidate, planned, options = {}) => {
    ...
    const workload = countWorkloadBlocks(wizardBaseBookings, coachUnavailable, planned, candidate.dateStr);
  ...
  const cleanupOldStudentAvailability = async () => {
    if (!wizardCutoff) {
      throw new Error('尚未設定學生選課截止時間');
    }

    const cutoffDate = new Date(wizardCutoff);
    if (Number.isNaN(cutoffDate.getTime())) {
      throw new Error('學生選課截止時間格式不正確');
    }

    const cleanupBoundary = new Date(cutoffDate);
    cleanupBoundary.setDate(cleanupBoundary.getDate() - 7);

    const oldQuery = query(
      collection(db, 'studentAvailable'),
      where('time', '<', Timestamp.fromDate(cleanupBoundary))
    );

    const oldSnapshot = await getDocs(oldQuery);
    if (oldSnapshot.empty) {
      return { deletedCount: 0, cleanupBoundary };
    }

    await Promise.all(
      oldSnapshot.docs.map((itemDoc) =>
        deleteDoc(doc(db, 'studentAvailable', itemDoc.id))
      )
    );

    return {
      deletedCount: oldSnapshot.size,
      cleanupBoundary
    };
  };

  const loadWizardAvailability = async ({ force = false } = {}) => {
    const targetNextWeekDays = getNextWeekDaysFromCurrentMonday(currentNow);
    const rangeStart = new Date(targetNextWeekDays[0]);
    rangeStart.setHours(0, 0, 0, 0);
    ...
    const rangeKey = `${formatLocalDate(rangeStart)}_${formatLocalDate(rangeEnd)}`;

    if (!force && wizardAvailableRangeKey === rangeKey) {
    ...
    setWizardAvailable(data);
    setWizardAvailableRangeKey(rangeKey);

    return data;
  };

  const generateWizardPlan = async () => {
    // 重新產生時暫時不要自動載回資料庫中的正式排課，
    // 並把既有下週正式排課視為「等待被新結果取代」，不占用排課時段。
    setSuppressPublishedAutoView(true);

    const regeneratingPublishedSchedule = isEditingPublishedSchedule;
    const originalPublishedItems = regeneratingPublishedSchedule
      ? publishedNextWeekBookings
      : [];
    ...
    if (!regeneratingPublishedSchedule) {
      setDeletedPublishedBookingIds([]);
    }

    if (!wizardCutoff) {
      alert('請先設定並儲存學生選課截止時間');
      return;
    }

    try {
      const cleanupResult = await cleanupOldStudentAvailability();

      console.log(
        `本次清理完成，共刪除 ${cleanupResult.deletedCount} 筆資料。`
      );
    } catch (error) {
      console.error('清理舊選課資料失敗：', error);
    ...
    // 按下按鈕後才一次讀取「下週全部學生」的可用時間
    const targetNextWeekDays = getNextWeekDaysFromCurrentMonday(currentNow);
    const targetNextWeekDateStrs = targetNextWeekDays.map(formatLocalDate);
    ...
    try {
      weekAvail = await loadWizardAvailability();
      setWizardSelectedStudentId('');
    } catch (error) {
      console.error('讀取下週全部學生選課資料失敗：', error);
      alert(`讀取下週學生選課資料失敗：${error.message || '未知錯誤'}`);
      return;
    }
    ...
    // 編輯正式排課時重新產生建議：依學生逐堂沿用原文件 ID。
    // 原本有、重新產生後沒有的課程，等按儲存時才刪除。
    if (regeneratingPublishedSchedule) {
      const remainingPublishedByStudent = new Map();

      originalPublishedItems.forEach((booking) => {
        const list = remainingPublishedByStudent.get(booking.studentId) || [];
        list.push(booking);
        remainingPublishedByStudent.set(booking.studentId, list);
      });

      finalPlanned = planned.map((item) => {
        const list = remainingPublishedByStudent.get(item.studentId) || [];
      ...
      const removedIds = [];
      remainingPublishedByStudent.forEach((list) => {
        list.forEach((booking) => removedIds.push(booking.id));
      });
      setDeletedPublishedBookingIds(removedIds);
      setIsEditingPublishedSchedule(true);
    }

    setWizardPlan(finalPlanned);
  ...
  const clearWizardPlan = () => {
    ...
    setWizardGeneratedAt('');
    setWizardAvailable([]);
    setWizardAvailableRangeKey('');
    setWizardSelectedStudentId('');
    setIsEditingPublishedSchedule(false);
    setIsViewingPublishedSchedule(false);
    // 清空後留在空白重排狀態，不要立刻從資料庫把舊正式排課載回來。
    setSuppressPublishedAutoView(true);
    setDeletedPublishedBookingIds([]);
    setDraggingWizardItem(null);
    setMobileMovingWizardItem(null);
  };

  const editPublishedSchedule = async () => {
    const targetDateStrs = getNextWeekDaysFromCurrentMonday(currentNow).map(formatLocalDate);

    const publishedItems = studentBookings
      .filter((booking) => targetDateStrs.includes(booking.dateStr))
      .map((booking) => ({
        ...booking,
        bookingId: booking.id,
        kind: 'planned',
    ...
    if (publishedItems.length === 0) {
      alert('下週目前沒有已送出的正式排課可以編輯');
      return;
    }

    try {
      await loadWizardAvailability();
    } catch (error) {
      console.error('讀取下週學生可用時間失敗：', error);
      alert(`讀取下週學生可用時間失敗：${error.message || '未知錯誤'}`);
      return;
    }
  ...
  const getWizardItemKey = (item) => {
    return `${item.studentId}-${item.dateStr}-${item.timeStr}-${item.venue}-${item.source || 'wizard'}`;
  };

  const removeWizardPlanItem = (itemToRemove) => {
    const confirmRemove = window.confirm(
      `確定要從建議課表移除 ${itemToRemove.name} 的課程嗎？\n\n` +
      `${itemToRemove.dateStr} ${itemToRemove.timeStr}｜${itemToRemove.venue}`
    );

    if (!confirmRemove) return;

    const removeKey = getWizardItemKey(itemToRemove);

    setWizardPlan((prev) =>
      prev.filter((item) => getWizardItemKey(item) !== removeKey)
    );

    if (isEditingPublishedSchedule && itemToRemove.bookingId) {
      setDeletedPublishedBookingIds((prev) =>
        prev.includes(itemToRemove.bookingId)
          ? prev
          : [...prev, itemToRemove.bookingId]
      );
    }

  };

  const moveWizardItemToCell = (itemToMove, targetDateStr, targetTimeStr) => {
  if (!itemToMove) return;

  const oldItem = itemToMove;
  ...
  const otherPlans = wizardPlan.filter((item) => {
    return getWizardItemKey(item) !== oldItem._dragKey;
  });

  const canOverrideStudentAvailability =
    oldItem.source === 'manual';

  if (!canOverrideStudentAvailability) {
    const studentCanTakeThisSlot = wizardAvailable.some((item) => {
      return (
  ...
  const handleDropWizardItem = (targetDateStr, targetTimeStr) => {
    moveWizardItemToCell(draggingWizardItem, targetDateStr, targetTimeStr);
  };

  const handleMobileWizardCellClick = (targetDateStr, targetTimeStr) => {
    if (!mobileMovingWizardItem) return;
    moveWizardItemToCell(mobileMovingWizardItem, targetDateStr, targetTimeStr);
  };

  const adoptWizardPlan = async () => {
    if (wizardPlan.length === 0 && deletedPublishedBookingIds.length === 0) {
      alert('目前沒有可儲存的排課內容');
      return;
    }
    ...
    try {
      await runTransaction(db, async (transaction) => {
        // 編輯正式排課時，被移除的課程會從資料庫刪除。
        deletedPublishedBookingIds.forEach((bookingId) => {
          transaction.delete(doc(db, 'studentBookings', bookingId));
        });
        ...
        wizardPlan.forEach((item) => {
          ...
          // 從「編輯已送出排課」載入的課程，直接更新原文件。
          if (isEditingPublishedSchedule && item.bookingId) {
            transaction.update(
              doc(db, 'studentBookings', item.bookingId),
              bookingData
            );
            return;
          }
          ...
          const exists = studentBookings.some((b) => {
            return (
              b.studentId === item.studentId &&
              b.dateStr === item.dateStr &&
              b.timeStr === item.timeStr &&
              b.venue === item.venue
            );
          });
  ...
  const wizardSelectedStudent = useMemo(() => {
    if (!wizardSelectedStudentId) return null;
    return students.find((student) => student.id === wizardSelectedStudentId) || null;
  }, [students, wizardSelectedStudentId]);

  const wizardSelectedStudentSlots = useMemo(() => {
    if (!wizardSelectedStudentId) return [];

    return wizardAvailable
      .filter(
        (slot) =>
          slot.studentId === wizardSelectedStudentId &&
  ...
  }, [wizardAvailable, wizardSelectedStudentId, nextWeekDateStrs]);

  const wizardSelectedStudentSlotsByDate = useMemo(() => {
    const grouped = new Map();

    wizardSelectedStudentSlots.forEach((slot) => {
      if (!grouped.has(slot.dateStr)) grouped.set(slot.dateStr, []);
      grouped.get(slot.dateStr).push(slot);
    });
  ...
  }, [wizardSelectedStudentSlots]);

  const wizardBookingsForAnalysis = useMemo(() => {
    // 小精靈畫面中的 wizardPlan 已代表下週目前要顯示／編輯的課程，
    // 所以排除資料庫中同一週的正式排課，避免課量重複計算。
    return studentBookings.filter(
      (booking) => !nextWeekDateStrs.includes(booking.dateStr)
    );
  }, [studentBookings, nextWeekDateStrs]);
  ...
  }, [students, wizardAvailable, nextWeekDateStrs]);

  const wizardPlannedCountByStudent = useMemo(() => {
    const countMap = new Map();

    wizardPlan.forEach((item) => {
      countMap.set(item.studentId, (countMap.get(item.studentId) || 0) + 1);
    });

    return countMap;
  }, [wizardPlan]);

  const getWizardPlannedCount = (studentId) => {
    return wizardPlannedCountByStudent.get(studentId) || 0;
  };

  const displayWizardUnscheduled = useMemo(() => {
    const uniqueMap = new Map();

    wizardUnscheduled.forEach((item) => {
      if (!item.studentId) return;
      if (!uniqueMap.has(item.studentId)) {
        uniqueMap.set(item.studentId, item);
      }
    });
    ...
    return Array.from(uniqueMap.values())
      .map((item) => {
        ...
        const needed = student ? getStudentWeeklyLessons(student) : 1;
        const plannedCount = getWizardPlannedCount(item.studentId);
        const missingCount = Math.max(0, needed - plannedCount);
        ...
        return {
          ...
          missingCount,
          reason:
            plannedCount > 0 && missingCount > 0
              ? `已在建議課表安排 ${plannedCount} 堂，尚缺 ${missingCount} 堂`
              : item.reason
        };
      })
      .filter((item) => {
        if (item.missingCount <= 0) return false;

        return wizardAvailable.some(
          (available) =>
            available.studentId === item.studentId &&
            nextWeekDateStrs.includes(available.dateStr)
        );
      })
      .sort((a, b) => (a.name || '').localeCompare(b.name || '', 'zh-Hant'));
  }, [
    wizardUnscheduled,
    students,
    wizardPlannedCountByStudent,
    wizardAvailable,
    nextWeekDateStrs
  ]);

  const displayWizardUnselected = useMemo(() => {
    return wizardUnselected
      .map((student) => {
        const needed = getStudentWeeklyLessons(student);
        const plannedCount = getWizardPlannedCount(student.id);
        const missingCount = Math.max(0, needed - plannedCount);
      ...
      .sort((a, b) => (a.name || '').localeCompare(b.name || '', 'zh-Hant'));
  }, [wizardUnselected, wizardPlannedCountByStudent]);

  const manualWizardStudents = useMemo(() => {
    const ids = new Set();

    displayWizardUnscheduled.forEach((item) => {
      if (item.studentId) ids.add(item.studentId);
    });

    displayWizardUnselected.forEach((student) => {
    ...
    return students
      ...
      .sort((a, b) => (a.name || '').localeCompare(b.name || '', 'zh-Hant'));
  }, [students, displayWizardUnscheduled, displayWizardUnselected]);
  ...
  const canEditWizardCalendar =
    !isViewingPublishedSchedule || isEditingPublishedSchedule;
            ...
            <div key={`${group.dateStr}-${group.venue}`} className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {group.slots.map((slot) => {
                  ...
                  const selectedForAdd = pendingAvailabilitySlots.some((item) =>
                    isSameSlot(item, slot)
                  );
                  const existingItem = studentAvailable.find((item) =>
                    isSameSlot(item, slot)
                  );
                  const selectedForCancellation = existingItem
                    ? pendingCancellationSlots.some(
                        (item) => item.id === existingItem.id
                      )
                    : false;
              ...
              {pendingAvailabilitySlots.length > 0 && (
                <>
                  <div className="text-sm font-bold mb-2">
                    已選擇新增 {pendingAvailabilitySlots.length} 個可上課時間
                  </div>
                  <div className="text-xs text-gray-600 mb-3 space-y-1 max-h-28 overflow-auto">
                    {pendingAvailabilitySlots.map((slot) => (
                      <div key={`${slot.dateStr}-${slot.venue}-${slot.timeStr}`}>
                        {slot.dateStr}｜{slot.venue}｜{slot.timeStr}
                      </div>
                    ))}
                  </div>
                </>
              ...
              {pendingCancellationSlots.length > 0 && (
                <>
                  <div className="text-sm font-bold text-red-600 mb-2">
                    已選擇取消 {pendingCancellationSlots.length} 個可上課時間
                  </div>
                  <div className="text-xs text-red-600 mb-3 space-y-1 max-h-28 overflow-auto">
                    {pendingCancellationSlots.map((slot) => (
                      <div key={slot.id}>
                        {slot.dateStr}｜{slot.venue}｜{slot.timeStr}
                      </div>
                    ))}
        ...
        <>
          ...
          {adminTab === 'wizard' && (
            <div>
              <div className="max-w-md mx-auto border rounded p-4 bg-gray-50 mb-5">
                ...
                {(!isViewingPublishedSchedule || isEditingPublishedSchedule) && (
                  <>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      產生建議時，系統會自動刪除「選課截止時間往前 7 天」以前的學生選課資料。
                    </p>
                  </>
                )}
                ...
                {publishedNextWeekBookings.length > 0 &&
                  !isEditingPublishedSchedule && (
                    <button
                      onClick={editPublishedSchedule}
                      className="w-full py-2 rounded bg-blue-600 text-white mt-2"
                    >
                      編輯已送出排課
                    </button>
                  )}
                ...
                {isEditingPublishedSchedule && (
                  <p className="text-xs text-blue-700 mt-2">
                    已進入正式排課編輯模式。可拖曳課程調整時間；移除課程後，按「儲存已送出排課變更」才會同步刪除資料庫中的正式排課。
                  </p>
                )}
              </div>
              ...
              {isMobileDevice && mobileMovingWizardItem && (
                <div className="mb-3 rounded-lg border border-indigo-300 bg-indigo-50 p-3">
                  <div className="font-medium text-indigo-800">
                    正在移動：{mobileMovingWizardItem.name}
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => setMobileMovingWizardItem(null)}
                        ...
                        {nextWeekDateStrs.map((dateStr) => {
                          ...
                          return (
                            <td
                              ...
                              onContextMenu={(e) => {
                                if (canEditWizardCalendar) {
                                  openWizardContextMenu(e, dateStr, row.timeStr);
                                }
                              }}
                              rowSpan={items.length > 0 ? 2 : 1}
                              className={`border-t border-r align-top relative ${
                                items.length > 0 ? 'p-0 h-20' : 'p-1 h-10'
                              } ${
                                isMobileDevice && mobileMovingWizardItem
                              ...
                              onDragOver={(e) => {
                                if (canEditWizardCalendar && !isMobileDevice) e.preventDefault();
                              }}
                              onDrop={() => {
                                if (canEditWizardCalendar && !isMobileDevice) {
                                  handleDropWizardItem(dateStr, row.timeStr);
                                }
                              }}
                              onClick={() => {
                                if (
                                  canEditWizardCalendar &&
                                  isMobileDevice &&
                                  mobileMovingWizardItem
                                ) {
                                  handleMobileWizardCellClick(dateStr, row.timeStr);
                                }
                              }}
                            >
                              {items.map((item) => (
                                <div
                                  key={getWizardItemKey(item)}
                                  draggable={canEditWizardCalendar && !isMobileDevice}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setWizardSelectedStudentId(item.studentId);
                                  }}
                                  onDragStart={() => {
                                    if (!canEditWizardCalendar || isMobileDevice) return;
                                    setDraggingWizardItem({
                                      ...item,
                                      _dragKey: getWizardItemKey(item)
                                    });
                                  }}
                                  onDragEnd={() => {
                                    if (canEditWizardCalendar && !isMobileDevice) {
                                  ...
                                  className={`relative w-full h-full min-h-20 px-2 py-2 pr-8 text-xs flex flex-col justify-center ${
                                    canEditWizardCalendar
                                ...
                                >
                                  {canEditWizardCalendar && (
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        e.preventDefault();
                                        removeWizardPlanItem(item);
                                      }}
                                      onMouseDown={(e) => e.stopPropagation()}
                                      className="absolute top-1 right-1 w-5 h-5 rounded-full bg-white border text-gray-500 hover:bg-red-50 hover:text-red-600 flex items-center justify-center leading-none z-10"
                                      title="移除此課程"
                                    >
                                      ×
                                    </button>
                                  ...
                                  <div className="font-semibold pr-4 leading-tight">{item.name}</div>
                                  <div className="mt-1 text-gray-700 leading-tight">
                                    {item.timeStr}-{addMinutesToTimeStr(item.timeStr, CLASS_DURATION)}
                                  </div>
                                  <div className="text-gray-600 leading-tight">{item.venue}</div>

                                  {isMobileDevice && canEditWizardCalendar && (
                                    <button
                                      type="button"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        ...
                                        if (mobileMovingWizardItem?._dragKey === movingItem._dragKey) {
                                          setMobileMovingWizardItem(null);
                                        } else {
                                          setMobileMovingWizardItem(movingItem);
                                        }
                                      }}
                                      className={`absolute bottom-1 right-1 w-7 h-7 rounded-full border flex items-center justify-center text-base z-10 ${
                                        mobileMovingWizardItem?._dragKey === getWizardItemKey(item)
                                          ? 'bg-indigo-600 text-white border-indigo-700'
                                          : 'bg-white text-gray-600 border-gray-300'
                                      }`}
                                      title="移動課程"
                                      aria-label={`移動 ${item.name} 的課程`}
                                    >
                                      ⚙
                                    </button>
                                  )}
              ...
              <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
                <div className="border rounded p-4 bg-white">
                  <h2 className="font-bold mb-2">該學生已選下週可上課時間</h2>

                  {!wizardSelectedStudentId && (
                    <p className="text-sm text-gray-400">
                      請點選上方課表中的學生姓名。
                    </p>
                  )}
                  ...
                  {wizardSelectedStudentId && (
                    <>
                      <div className="font-medium mb-2">
                        {wizardSelectedStudent?.name || '未命名學生'}
                      </div>

                      {wizardSelectedStudentSlotsByDate.length === 0 ? (
                        <p className="text-sm text-gray-400">
                          這位學生下週沒有送出可上課時間。
                        </p>
                      ) : (
                        <div className="divide-y divide-gray-200">
                          {wizardSelectedStudentSlotsByDate.map((group) => (
                            <div key={group.dateStr} className="py-3 first:pt-0 last:pb-0">
                              <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
                                <div className="sm:w-36 shrink-0">
                                  <div className="font-semibold text-gray-800">{group.dateStr}</div>
                                  <div className="text-xs text-gray-500">{group.weekday}</div>
                                </div>

                                <div className="flex-1 space-y-1">
                                  {group.slots.map((slot) => (
                                    <div
                                      key={`${slot.id}-${slot.timeStr}-${slot.venue}`}
                                      className="flex items-center gap-3 text-sm"
                                    >
                                      <span className="font-medium text-indigo-700 min-w-[105px]">
                                        {slot.timeStr}-{addMinutesToTimeStr(slot.timeStr, CLASS_DURATION)}
                                      </span>
                                      <span className="text-gray-600">{slot.venue}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                ...
                <div className="space-y-4">
                  <div className="border rounded p-4 bg-white">
                    ...
                    <div className="space-y-2">
                      {displayWizardUnscheduled.length === 0 && (
                        <p className="text-sm text-gray-400">
                          目前沒有未排到的學生
                        </p>
                      )}

                      {displayWizardUnscheduled.map((item, idx) => (
                        <div
                  ...
                  <div className="border rounded p-4 bg-white">
                    ...
                    <div className="space-y-2">
                      {displayWizardUnselected.length === 0 && (
                        <p className="text-sm text-gray-400">
                          目前所有學生都已填寫下週可上課時間
                        </p>
                      )}

                      {displayWizardUnselected.map((student) => (
                        <div
                          key={student.id}
