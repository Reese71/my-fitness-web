...
const App = () => {
  ...
  const getTimePeriodPreferenceScore = (candidate) => {
    ...
    // 下午最優先：12:00 ~ 17:59
    if (minutes >= 12 * 60 && minutes < 18 * 60) return -40;

    // 早上第二優先：08:00 ~ 11:59
    if (minutes >= 8 * 60 && minutes < 12 * 60) return 0;

    // 晚上最後：18:00 之後
    if (minutes >= 18 * 60) return 40;
  ...
  const scoreCandidate = (candidate, planned, options = {}) => {
    ...
    // 盡量不要太早開始。
    // 08:00 / 08:30 很不優先，09:00～09:30 稍微不優先，10:00 後較理想。
    if (candidate.totalMinutes < 9 * 60) {
      score += 10;
    } else if (candidate.totalMinutes < 10 * 60) {
      score += 20;
    }
    // 盡量排 09:00 之後。
    // 不是禁止 08:00 / 08:30，只是降低優先權。
    if (candidate.totalMinutes < 9 * 60) {
      score += 20;
    }
  ...
  const generateWizardPlan = () => {
    ...
    // 只排「下週有填可用時間」的學生
    const studentsWithAvailability = students.filter((student) =>
      weekAvail.some((item) => item.studentId === student.id)
    );

    const studentsByNeed = [...studentsWithAvailability].sort((a, b) => {
      const aNeeded = getStudentWeeklyLessons(a);
      ...
      // 第一優先：可用時間數量剛好等於每週堂數
      const aExact = aCount === aNeeded ? 0 : 1;
      const bExact = bCount === bNeeded ? 0 : 1;
