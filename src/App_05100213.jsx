...
const App = () => {
  ...
  const generateWizardPlan = () => {
    const weekAvail = studentAvailable.filter((item) => currentWeekDateStrs.includes(item.dateStr));
    const planned = [];
    const unscheduled = [];
  ...
  const wizardRestByDay = currentWeekDateStrs.map((dateStr) => ({
    dateStr,
      ...
      {mode === 'student' && (
        <div className="max-w-md mx-auto">
          <div className="border rounded p-3 mb-5">
            <div className="font-bold mb-3 text-center">本週與下週，可多選日期</div>
          ...
          {adminTab === 'schedule' && (
            <div className="max-w-md mx-auto">
              ...
              <div className="space-y-2 mb-5">
                {adminDaySchedule.length === 0 && <p className="text-sm text-gray-400">此日期沒有任何行程</p>}
                {adminDaySchedule.map((item) => (
                  <div key={`${item.kind}-${item.id}`} className={`border rounded p-3 flex justify-between items-center ${item.kind === 'unavailable' ? 'bg-gray-50' : 'bg-white'}`}>
                    <div>
                      <div className="font-medium">{item.venue}｜{item.timeStr}</div>
                      <div className="text-sm text-gray-500">{item.kind === 'booking' ? `學生：${item.name || '未填姓名'}` : item.typeLabel}</div>
                    </div>
                    {item.kind === 'booking' ? (
                      <button onClick={() => handleCancelBooking(item)} className="text-sm px-3 py-1 rounded bg-red-100 text-red-600">取消</button>
                    ) : (
                      <button onClick={() => handleDeleteUnavailable(item)} className="text-sm px-3 py-1 rounded bg-red-100 text-red-600">刪除</button>
                    )}
                  </div>
                ))}
              </div>
          ...
          {adminTab === 'students' && (
            <div className="max-w-md mx-auto">
              ...
              <div className="border rounded p-4 bg-gray-50 mb-5">
                ...
                <input value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} className="w-full p-2 border rounded mb-3" placeholder="請輸入學生姓名" />

                <label className="block text-sm mb-1">價格</label>
                <input value={newStudentPrice} onChange={(e) => setNewStudentPrice(e.target.value)} className="w-full p-2 border rounded mb-3" placeholder="可先空白" />

                <label className="block text-sm mb-1">每週堂數</label>
                <input type="number" min="1" value={newStudentWeeklyLessons} onChange={(e) => setNewStudentWeeklyLessons(e.target.value)} className="w-full p-2 border rounded mb-3" />

                <div className="text-sm mb-2">上課場館</div>
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {venues.map((venue) => (
                    <label key={venue} className="flex items-center gap-2 text-sm border rounded p-2">
                      <input type="checkbox" checked={newStudentVenues.includes(venue)} onChange={() => handleToggleNewStudentVenue(venue)} />
                      {venue}
                    </label>
                  ))}
                </div>
                <button onClick={handleAddStudent} className="w-full py-2 rounded bg-indigo-600 text-white">新增學生</button>
              </div>

              <h2 className="font-bold mb-3">學生管理</h2>
              <div className="text-sm text-gray-500 mb-3">第 {studentPage} / {totalStudentPages} 頁，每頁 10 位學生</div>
              <div className="space-y-3">
                {currentStudents.map((student) => (
                  <div key={student.id} className="border rounded p-3">
                    <div className="font-bold mb-1">{student.name || '未命名學生'}</div>
                    <div className="text-sm text-gray-500 mb-2">價格：{student.price || ''}</div>
                    <label className="block text-sm mb-1">每週堂數</label>
                    <input type="number" min="1" defaultValue={getStudentWeeklyLessons(student)} onBlur={(e) => updateStudentWeeklyLessons(student, e.target.value)} className="w-full p-2 border rounded mb-2" />
                    <div className="text-sm mb-2">上課場館：{student.venues?.length ? student.venues.join('、') : '尚未設定'}</div>
                    <div className="grid grid-cols-2 gap-2">
                      {venues.map((venue) => (
                        <label key={venue} className="flex items-center gap-2 text-sm border rounded p-2">
                          <input type="checkbox" checked={Array.isArray(student.venues) && student.venues.includes(venue)} onChange={() => handleToggleStudentVenue(student, venue)} />
                          {venue}
                        </label>
                      ))}
                    </div>
              ...
              <div className="flex gap-2 mt-4">
                <button disabled={studentPage <= 1} onClick={() => setStudentPage((p) => Math.max(1, p - 1))} className={`flex-1 py-2 rounded ${studentPage <= 1 ? 'bg-gray-100 text-gray-300' : 'bg-gray-100'}`}>上一頁</button>
                <button disabled={studentPage >= totalStudentPages} onClick={() => setStudentPage((p) => Math.min(totalStudentPages, p + 1))} className={`flex-1 py-2 rounded ${studentPage >= totalStudentPages ? 'bg-gray-100 text-gray-300' : 'bg-gray-100'}`}>下一頁</button>
              </div>
            </div>
          )}
          ...
          {adminTab === 'availability' && (
            <div className="max-w-md mx-auto">
              ...
              <div className="border rounded p-4 bg-gray-50 mb-5">
                ...
                <input type="date" value={adminDate} onChange={(e) => setAdminDate(e.target.value)} className="w-full p-2 border rounded mb-3" />

                <label className="block text-sm mb-1">時間</label>
                <input type="time" value={adminTime} onChange={(e) => setAdminTime(e.target.value)} step="1800" className="w-full p-2 border rounded mb-3" />

                <label className="block text-sm mb-1">場地</label>
                <select value={adminVenue} onChange={(e) => setAdminVenue(e.target.value)} className="w-full p-2 border rounded mb-3">
                  {venues.map((venue) => <option key={venue} value={venue}>{venue}</option>)}
                </select>

                <label className="block text-sm mb-1">類型</label>
                <select value={adminUnavailableType} onChange={(e) => setAdminUnavailableType(e.target.value)} className="w-full p-2 border rounded mb-3">
                  {unavailableTypes.map((type) => <option key={type} value={type}>{type}</option>)}
                </select>

                <p className="text-xs text-gray-500 mb-3">私事只會擋住不可預約時間；教學、工作、其他會算進一天 8 堂工作量。</p>
                <button onClick={handleAddCoachUnavailable} className="w-full py-2 rounded bg-indigo-600 text-white">新增不可預約</button>
              </div>

              {editingUnavailableId && (
                <div className="border rounded p-4 bg-gray-50 mb-5">
                  <h3 className="font-bold mb-3">編輯不可預約時間</h3>
                  <input type="date" value={editingDate} onChange={(e) => setEditingDate(e.target.value)} className="w-full p-2 border rounded mb-3" />
                  <input type="time" value={editingTime} onChange={(e) => setEditingTime(e.target.value)} step="1800" className="w-full p-2 border rounded mb-3" />
                  <select value={editingVenue} onChange={(e) => setEditingVenue(e.target.value)} className="w-full p-2 border rounded mb-3">
                    {venues.map((venue) => <option key={venue} value={venue}>{venue}</option>)}
                  </select>
                  <select value={editingType} onChange={(e) => setEditingType(e.target.value)} className="w-full p-2 border rounded mb-3">
                    {unavailableTypes.map((type) => <option key={type} value={type}>{type}</option>)}
                  </select>
                  <button onClick={handleUpdateUnavailable} className="w-full py-2 rounded bg-indigo-600 text-white">儲存修改</button>
                  <button onClick={() => setEditingUnavailableId('')} className="w-full py-2 rounded bg-gray-100 mt-2">取消編輯</button>
                </div>
              )}

              <h2 className="font-bold mb-3">未來兩週教練不能的時間</h2>
              <div className="space-y-2">
                {nextTwoWeekUnavailable.length === 0 && <p className="text-sm text-gray-400">未來兩週沒有設定教練不可預約時間</p>}
                {nextTwoWeekUnavailable.map((item) => (
                  <div key={item.id} className="border rounded p-3 flex justify-between items-center">
                    <div>
                      <div className="font-medium">{item.dateStr}｜{item.venue}｜{item.timeStr}</div>
                      <div className="text-sm text-gray-500">類型：{item.type || '私事'}</div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => startEditUnavailable(item)} className="text-sm px-3 py-1 rounded bg-gray-100">編輯</button>
                      <button onClick={() => handleDeleteUnavailable(item)} className="text-sm px-3 py-1 rounded bg-red-100 text-red-600">刪除</button>
                    </div>
                  </div>
                ))}
            ...
            <div>
              <div className="max-w-md mx-auto border rounded p-4 bg-gray-50 mb-5">
                ...
                <p className="text-sm text-gray-600 mb-3">
                  依據學生本週填寫的可上課時間，自動嘗試安排課表。排序目標：先讓大家上到課，再減少教練奔波，並盡量安排休息。
                </p>
                <button onClick={generateWizardPlan} className="w-full py-2 rounded bg-indigo-600 text-white">產生本週排課建議</button>
                <button onClick={adoptWizardPlan} className="w-full py-2 rounded bg-green-600 text-white mt-2">採用建議並寫入正式排課</button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-3 mb-5">
                {wizardRestByDay.map((item) => (
                  <div key={item.dateStr} className="border rounded p-3 bg-white">
                    <div className="font-bold">{item.dateStr}</div>
                    <div className="text-sm text-gray-600">課量：{item.count}</div>
                    <div className={`text-sm ${item.level === 'bad' ? 'text-red-600' : item.level === 'tight' ? 'text-orange-600' : 'text-gray-600'}`}>{item.text}</div>
                  </div>
                ))}
              </div>
                  ...
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      {currentWeekDays.map((day) => (
                        <th key={formatLocalDate(day)} className="border p-2">{formatLocalDate(day)}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {Array.from({ length: 15 }, (_, i) => 8 + i).map((hour) => (
                      <tr key={hour}>
                        <td className="border p-2 font-medium">{toTimeStr(hour)}</td>
                        {currentWeekDateStrs.map((dateStr) => {
                          const items = getWizardCellItems(dateStr, hour);
                          return (
                            <td key={`${dateStr}-${hour}`} className="border p-2 align-top h-20">
                              {items.map((item) => (
                                <div key={`${item.studentId}-${item.dateStr}-${item.timeStr}-${item.venue}`} className="rounded bg-indigo-50 border border-indigo-100 p-1 mb-1">
                                  <div className="font-bold">{item.timeStr} {item.name}</div>
                                  <div>{item.venue}</div>
                                </div>
                              ))}
                            </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                <div>
                  ...
                  <div className="space-y-2">
                    {wizardPlan.length === 0 && <p className="text-sm text-gray-400">尚未產生建議</p>}
                    {wizardPlan.map((item) => (
                      <div key={`${item.studentId}-${item.dateStr}-${item.timeStr}-${item.venue}`} className="border rounded p-3">
                        <div className="font-medium">{item.dateStr}｜{item.timeStr}｜{item.venue}</div>
                        <div className="text-sm text-gray-500">學生：{item.name}</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2">
                    {wizardUnscheduled.length === 0 && <p className="text-sm text-gray-400">目前沒有未排到的學生</p>}
                    {wizardUnscheduled.map((item, idx) => (
                      <div key={`${item.studentId}-${idx}`} className="border rounded p-3 bg-red-50">
